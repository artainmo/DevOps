...
Using EC2 Spot instances can save costs up to 90% over on-demand instances. The catch with Spot instances is that they can be interrupted at any time. Checkpoints can be used in S3 for training to pick up where it left off. Waiting for a Spot instance to become available can increase training time.
